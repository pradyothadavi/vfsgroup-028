...
void display(struct node* bst_root)
{
	if(bst_root==NULL)
	printf("empty tree\n");
	else
...
void insert(struct fileDescriptor* s_inode)
{
...
struct node* search(struct node* bst_root,struct fileDescriptor* full_Path)
{	
	...
	{ 	
			if(strcmp(fullPath,temp->bst_full_path)<0)
				return search(temp->lc,full_Path);
			if(strcmp(fullPath,temp->bst_full_path)>0)
		 		return search(temp->rc,full_Path);
...
void transplant(struct node* t1, struct node* t2)
{	
	if(t1->parent==NULL)
	{
	bst_root=t2;
...
void delete(struct fileDescriptor* fullPath)
{	
	struct node* loc=(struct node*)malloc(sizeof(struct node));
	loc=search(bst_root,fullPath);
	if(loc!=NULL)
	{
		if(loc->lc==NULL)
		{
		transplant(loc,loc->rc); 
		}
		else 
			{
			if(loc->rc==NULL)
			transplant(loc,loc->lc);
			else 
			{
				struct node * loc1=min(loc->rc);
		 		if(loc1->parent!=loc)
				{
				transplant(loc1,loc1->rc);
				loc1->rc=loc->rc;
				loc1->rc->parent=loc1;
				}
				transplant(loc,loc1);
